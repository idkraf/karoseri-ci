...
class Settings_model extends CI_Model {
    ...
    public function update_company($id, $name, $phone, $email, $address, $city, $region, $country, $postbox, $taxid, $data_share, $foundation) {
        ...
        $this->db->set($data);
        $this->db->where('id', $id);
        if ($this->db->update('smartpos_system')) {
    ...
    public function update_smtp($host, $port, $auth, $auth_type, $username, $password, $sender) {
        ...
        $this->db->set($data);
        $this->db->where('id', 1);
        if ($this->db->update('smartpos_smtp')) {
            echo json_encode(array('status' => 'Success', 'message' =>
                $this->lang->line('UPDATED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
    ...
    public function add_term($title, $type, $term) {
        ...
        if ($this->db->insert('smartpos_terms', $data)) {
            echo json_encode(array('status' => 'Success', 'message' =>
                $this->lang->line('ADDED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
    ...
    public function edit_term($id, $title, $type, $term) {
        ...
        $this->db->set($data);
        $this->db->where('id', $id);
        if ($this->db->update('smartpos_terms', $data)) {
            echo json_encode(array('status' => 'Success', 'message' =>
                $this->lang->line('UPDATED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
    ...
    public function currency() {
        $this->db->select('smartpos_system.currency,univarsal_api.*');
        $this->db->from('smartpos_system');
        $this->db->where('univarsal_api.id', 4);
        $this->db->where('smartpos_system.id', 1);
        $this->db->join('univarsal_api', 'smartpos_system.id = 1', 'left');
        $query = $this->db->get();
        return $query->row_array();
    }
    ...
    public function update_currency($id, $currency, $thous_sep, $deci_sep, $decimal, $method, $roundoff = 'Off', $r_precision = 0) {
        ...
        $this->db->set($data);
        $this->db->where('id', $id);
        $this->db->update('smartpos_system');
        $data = array(
            'key1' => $deci_sep,
            'key2' => $thous_sep,
            'url' => $decimal,
    ...
    public function delete_terms($id) {
        ...
        $query = $this->db->get();
        if ($query->num_rows() > 1) {
            return $this->db->delete('smartpos_terms', array('id' => $id));
        } else {
            return false;
        }
    }
    ...
    public function delete_slab($id) {
        return $this->db->delete('smartpos_config', array('id' => $id, 'type' => 2));
    }
    ...
    public function custom_fields($id = 0) {
        ...
        if ($id) {
            $this->db->select('*');
            $this->db->from('smartpos_custom_fields');
            $this->db->where('id', $id);
            $query = $this->db->get();
            return $query->row_array();
        } else {
    ...
    public function custom_field_add($f_name, $f_type, $f_module, $f_view, $f_required, $f_placeholder, $f_description) {
        ...
        if ($this->db->insert('smartpos_custom_fields', $data)) {
            echo json_encode(array('status' => 'Success', 'message' =>
    ...
    public function custom_field_edit($id, $f_name, $f_view, $f_required, $f_placeholder, $f_description) {
        ...
        $this->db->set($data);
        $this->db->where('id', $id);
        if ($this->db->update('smartpos_custom_fields')) {
            echo json_encode(array('status' => 'Success', 'message' =>
                $this->lang->line('UPDATED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
