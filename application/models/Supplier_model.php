...
class Supplier_model extends CI_Model {
    ...
    var $inv_order = array('smartpos_purchase.tid' => 'desc');
    ...
    public function add($name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $taxid) {
        ...
        if ($this->db->insert('smartpos_supplier', $data)) {
    ...
    public function edit($id, $name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $taxid) {
        ...
        if ($this->db->update('smartpos_supplier')) {
            echo json_encode(array('status' => 'Success', 'message' =>
    ...
    public function group_list() {
        $query = $this->db->query("SELECT c.*,p.pc FROM smartpos_cust_group AS c LEFT JOIN ( SELECT gid,COUNT(gid) AS pc FROM smartpos_supplier GROUP BY gid) AS p ON p.gid=c.id");
        return $query->result_array();
    }

    public function delete($id) {

        return $this->db->delete('smartpos_supplier', array('id' => $id));
    }
    ...
    private function _inv_datatables_query($id) {
        $this->db->select('smartpos_purchase.*');
        $this->db->from('smartpos_purchase');
        $this->db->where('smartpos_purchase.csd', $id);
