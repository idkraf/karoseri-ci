...
class Units_model extends CI_Model {
    ...
    public function units_list() {
        $query = $this->db->query("SELECT * FROM smartpos_units WHERE type=0 ORDER BY id DESC");
    ...
    public function create($name, $code) {
        ...
        if ($this->db->insert('smartpos_units', $data)) {
    ...
    public function edit($id, $name, $code) {
        ...
        if ($this->db->update('smartpos_units')) {
            echo json_encode(array('status' => 'Success', 'message' =>
    ...
    public function variations_list() {
        $query = $this->db->query("SELECT * FROM smartpos_units WHERE type=1 ORDER BY id DESC");
        return $query->result_array();
    }
    ...
    public function variables_list() {
        //   $query = $this->db->query("SELECT * FROM smartpos_units WHERE type=2 ORDER BY id DESC");
        //    return $query->result_array();
