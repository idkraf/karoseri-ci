...
class Cronjob_model extends CI_Model {
    ...
    public function due_mail() {
        ...
        $this->db->select('smartpos_invoices.*,smartpos_customers.name,smartpos_customers.email');
        $this->db->from('smartpos_invoices');
        $this->db->where('DATE(smartpos_invoices.invoiceduedate)<=', $duedate);
        $this->db->where('smartpos_invoices.status', 'due');
    ...
    public function reports() {
        ...
        $this->db->delete('smartpos_reports', array('year' => $year));

        $query = $this->db->query("SELECT MONTH(invoicedate) AS month,YEAR(invoicedate) AS year,COUNT(tid) AS invoices,SUM(total) AS sales,SUM(items) AS items FROM smartpos_invoices WHERE (YEAR(invoicedate)='$year') GROUP BY MONTH(invoicedate)");
        $arrayA = $query->result_array();

        $query = $this->db->query("SELECT MONTH(date) AS month,YEAR(date) AS year,SUM(credit) AS income,SUM(debit) AS expense FROM smartpos_transactions WHERE (YEAR(date)='$year') GROUP BY MONTH(date)");
        ...
        $this->db->insert_batch('smartpos_reports', $batch);
...
//print_r($updateData);
        $this->db->update_batch('smartpos_currencies', $updateData, 'id');
    }
    ...
    public function subs() {
        ...
        if ($config == 0) {
            ...
            foreach ($result as $row) {
                ...
                $this->db->insert_batch('smartpos_invoice_items', $productlist_p);
                ...
                $this->db->where('smartpos_invoice_items.tid', $last_id);
                $query = $this->db->get();
            ...
            if ($result) {
                ...
                $this->db->insert_batch('smartpos_invoices', $invoice_list);
                if ($this->db->update_batch('smartpos_invoices', $old_invoice_list, 'id')) {
                    return true;
                } else {
                    return false;
                }
    ...
    public function stock() {
        ...
        $query = $this->db->query("SELECT product_name,product_price,qty,unit FROM smartpos_products WHERE qty<=alert ORDER BY product_name");
        $result = $query->result_array();
    ...
    public function expiry() {
        ...
        $query = $this->db->query("SELECT product_name,product_price,qty,unit,expiry FROM smartpos_products WHERE DATE(expiry)<='" . date('Y-m-d') . "' ORDER BY product_name");
        $result = $query->result_array();
