...
class Productgroups_model extends CI_Model {
    ...
    var $table = 'smartpos_product_groups';
    var $column_order = array(null, 'smartpos_product_groups.id', null); //set column field database for datatable orderable
    var $column_search = array(null, 'smartpos_product_groups.name', null);  //set column field database for datatable searchable
    var $order = array('smartpos_product_groups.id' => 'desc'); // default order
    ...
    public function addnew($warehouse, $group_name, $taxrate, $v_stock, $products_l, $image, $disrate) {
        ...
        if ($ware_valid['loc'] == $this->aauth->get_user()->loc or!$this->aauth->get_user()->loc) {
            ...
            $this->db->trans_start();
            if ($this->db->insert('smartpos_product_groups', $data)) {
            ...
            if (is_array($products_l)) {
                ...
                foreach ($products_l as $key => $value) {
                    if (numberClean($v_stock[$key]) > 0.00) {
                        ...
                        $data['rid'] = $value;
                        $this->db->insert('smartpos_group_relation', $data);
                    }
                }
            }
