...
class Manager_model extends CI_Model {
    ...
    private function _project_datatables_query($cday = '') {
        ...
        $this->db->where('smartpos_project_meta.meta_data', $this->aauth->get_user()->id);
        if ($cday) {
    ...
    public function project_stats($project) {
        ...
        $query = $this->db->query("SELECT
				COUNT(IF( smartpos_projects.status = 'Waiting', smartpos_projects.id, NULL)) AS Waiting,
				COUNT(IF( smartpos_projects.status = 'Progress', smartpos_projects.id, NULL)) AS Progress,
				COUNT(IF( smartpos_projects.status = 'Finished', smartpos_projects.id, NULL)) AS Finished			
				FROM smartpos_projects LEFT JOIN smartpos_project_meta ON smartpos_project_meta.pid=smartpos_projects.id WHERE smartpos_project_meta.meta_key=19 AND smartpos_project_meta.meta_data=" . $this->aauth->get_user()->id . "");
    ...
    public function task_stats($id) {
        $query = $this->db->query("SELECT
				...
				FROM smartpos_todolist WHERE related=1 AND rid=$id AND aid=" . $this->aauth->get_user()->id . "");
    ...
    public function explore($id) {
        ...
        $this->db->join('smartpos_invoices', 'smartpos_project_meta.meta_data = smartpos_invoices.tid', 'left');
        $query = $this->db->get();
