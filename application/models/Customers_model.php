...
class Customers_model extends CI_Model {
    ...
    var $table = 'smartpos_customers';
    var $column_order = array(null, 'smartpos_customers.name', 'smartpos_customers.address', 'smartpos_customers.email', 'smartpos_customers.phone', null);
    var $column_search = array('smartpos_customers.name', 'smartpos_customers.phone', 'smartpos_customers.address', 'smartpos_customers.city', 'smartpos_customers.email', 'smartpos_customers.docid');
    ...
    var $inv_order = array('smartpos_invoices.tid' => 'desc');
    var $qto_order = array('smartpos_quotes.tid' => 'desc');
    var $notecolumn_order = array(null, 'title', 'cdate', null);
    var $notecolumn_search = array('id', 'title', 'cdate');
    var $pcolumn_order = array('smartpos_projects.status', 'smartpos_projects.name', 'smartpos_projects.edate', 'smartpos_projects.worth', null);
    var $pcolumn_search = array('smartpos_projects.name', 'smartpos_projects.edate', 'smartpos_projects.status');
    ...
    private function _get_datatables_query($id = '') {
        ...
        if ($due) {
            ...
            $this->db->select('smartpos_customers.*,SUM(smartpos_invoices.total) AS total,SUM(smartpos_invoices.pamnt) AS pamnt');
            $this->db->from('smartpos_invoices');
            $this->db->where('smartpos_invoices.status!=', 'paid');
            $this->db->join('smartpos_customers', 'smartpos_customers.id = smartpos_invoices.csd', 'left');
            if ($this->aauth->get_user()->loc) {
                $this->db->where('smartpos_customers.loc', $this->aauth->get_user()->loc);
            } elseif (!BDATA) {
                $this->db->where('smartpos_customers.loc', 0);
            }
            if ($id != '') {
                $this->db->where('smartpos_customers.gid', $id);
            }
    ...
    public function details($custid, $loc = true) {
        $this->db->select('smartpos_customers.*,users.lang');
        $this->db->from($this->table);
        $this->db->join('users', 'users.cid=smartpos_customers.id', 'left');
        $this->db->where('smartpos_customers.id', $custid);
        if ($loc) {
            if ($this->aauth->get_user()->loc) {
                $this->db->where('smartpos_customers.loc', $this->aauth->get_user()->loc);
            } elseif (!BDATA) {
    ...
    public function add($name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $customergroup, $taxid, $name_s, $phone_s, $email_s, $address_s, $city_s, $region_s, $country_s, $postbox_s, $language = '', $create_login = true, $password = '', $docid = '', $custom = '', $discount = 0) {
        ...
        if (!$valid['email']) {
            ...
            if (!$discount) {
                ...
                $this->db->from('smartpos_cust_group');
            ...
            if ($this->db->insert('smartpos_customers', $data)) {
    ...
    public function edit($id, $name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $customergroup, $taxid, $name_s, $phone_s, $email_s, $address_s, $city_s, $region_s, $country_s, $postbox_s, $docid = '', $custom = '', $language = '', $discount = 0) {
        ...
        if ($this->db->update('smartpos_customers')) {
            $data = array(
                'name' => $name,
                'email' => $email,
                'lang' => $language
    ...
    public function editpicture($id, $pic) {
        ...
        if ($this->db->update('smartpos_customers') AND $result['picture'] != 'example.png') {
    ...
    public function group_list() {
        $whr = "";
        if ($this->aauth->get_user()->loc) {
            $whr = "WHERE (smartpos_customers.loc=" . $this->aauth->get_user()->loc . " ) ";
            if (BDATA)
                $whr = "WHERE (smartpos_customers.loc=" . $this->aauth->get_user()->loc . " OR smartpos_customers.loc=0 ) ";
        } elseif (!BDATA) {
            $whr = "WHERE  smartpos_customers.loc=0  ";
        }

        $query = $this->db->query("SELECT c.*,p.pc FROM smartpos_cust_group AS c LEFT JOIN ( SELECT gid,COUNT(gid) AS pc FROM smartpos_customers $whr GROUP BY gid) AS p ON p.gid=c.id");
    ...
    public function delete($id) {
        ...
        if ($this->aauth->get_user()->loc) {
            $this->db->delete('smartpos_customers', array('id' => $id, 'loc' => $this->aauth->get_user()->loc));
        } elseif (!BDATA) {
            $this->db->delete('smartpos_customers', array('id' => $id, 'loc' => 0));
        } else {
            $this->db->delete('smartpos_customers', array('id' => $id));
        }
        ...
        if ($this->db->affected_rows()) {
            ...
            $this->db->delete('smartpos_notes', array('fid' => $id, 'rid' => 1));
            //docs
            $this->db->select('filename');
            $this->db->from('smartpos_documents');
            $this->db->where('id', $id);
            $query = $this->db->get();
            $result = $query->row_array();
            if ($this->db->delete('smartpos_documents', array('fid' => $id, 'rid' => 1))) {
    ...
    private function _inv_datatables_query($id, $tyd = 0) {
        ...
        $this->db->from('smartpos_invoices');
        $this->db->where('smartpos_invoices.csd', $id);
        if ($this->aauth->get_user()->loc) {
    ...
    private function _qto_datatables_query($id, $tyd = 0) {
        $this->db->select('smartpos_quotes.*');
        $this->db->from('smartpos_quotes');
        $this->db->where('smartpos_quotes.csd', $id);
        if ($this->aauth->get_user()->loc) {
    ...
    private function _project_datatables_query($cday = '') {
        ...
        $this->db->where('smartpos_projects.cid=', $cday);
    ...
    public function editnote($id, $title, $content, $cid) {
        ...
        if ($this->db->update('smartpos_notes')) {
    ...
    function adddocument($title, $filename, $cid) {
        ...
        return $this->db->insert('smartpos_documents', $data);
    }
    ...
    function deletedocument($id, $cid) {
        ...
        $result = $query->row_array();
        $this->db->trans_start();
        if ($this->db->delete('smartpos_documents', array('id' => $id, 'fid' => $cid, 'rid' => 1))) {
