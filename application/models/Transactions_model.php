...
class Transactions_model extends CI_Model {
    ...
    private function _get_datatables_query() {
        $this->db->select('smartpos_transactions.*,smartpos_transactions.id as id');
    ...
    public function categories() {
        $this->db->select('*');
        $this->db->from('smartpos_trans_cat');
        $query = $this->db->get();
    ...
    public function addcat($name) {
        ...
        return $this->db->insert('smartpos_trans_cat', $data);
    }
    ...
    public function addtrans($payer_id, $payer_name, $pay_acc, $date, $debit, $credit, $pay_type, $pay_cat, $paymethod, $note, $eid, $loc = 0, $ty = 0) {
        ...
        if ($pay_acc > 0) {
            ...
            if ($account) {
                ...
                return $this->db->insert('smartpos_transactions', $data);
            }
        }
    }
    ...
    public function delt($id) {
        ...
        $this->db->delete('smartpos_transactions', array('id' => $id));
    ...
    public function cview($id, $ext = 0) {
        ...
        if ($ext == 1) {
            $this->db->select('*');
            $this->db->from('smartpos_supplier');
            $this->db->where('id', $id);
            if ($this->aauth->get_user()->loc) {
                $this->db->group_start();
                $this->db->where('loc', $this->aauth->get_user()->loc);
            ...
            $this->db->select('smartpos_employees.*,smartpos_users.email');
    ...
    public function cat_update($id, $cat_name) {
        ...
        if ($this->db->update('smartpos_trans_cat')) {
            return true;
        } else {
            return false;
        }
