...
class Search extends CI_Controller {
    ...
    public function invoice() {
        ...
        if ($tid) {
            $query = $this->db->query("SELECT tid FROM smartpos_invoices WHERE $whr (UPPER(tid)  LIKE '" . $tid . "%')  LIMIT 4");
    ...
    public function customer() {
        ...
        if ($name) {
            $query = $this->db->query("SELECT id,name,address,city,phone,email FROM smartpos_customers WHERE $whr (UPPER(name)  LIKE '%" . strtoupper($name) . "%' OR UPPER(phone)  LIKE '" . strtoupper($name) . "%') LIMIT 6");
    ...
    public function user() {
        ...
        if ($name != NULL) {
            $query = $this->db->query("SELECT id,username FROM smartpos_users WHERE $whr username  LIKE '%" . $name . "%' LIMIT 6");
    ...
    public function customer_select() {
        ...
        if ($name) {
            $query = $this->db->query("SELECT id,name,address,city,phone,email FROM smartpos_customers WHERE $whr (UPPER(name)  LIKE '" . strtoupper($name['term']) . "%' OR UPPER(phone)  LIKE '" . strtoupper($name['term']) . "%') LIMIT 6");
    ...
    public function supplier_select() {
        ...
        if ($name) {
            $query = $this->db->query("SELECT id,name,address,city,phone,email FROM smartpos_supplier WHERE $whr (UPPER(name)  LIKE '" . strtoupper($name['term']) . "%' OR UPPER(phone)  LIKE '" . strtoupper($name['term']) . "%') LIMIT 6");
