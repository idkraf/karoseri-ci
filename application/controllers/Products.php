...
class Products extends CI_Controller {
    ...
    public function product_list() {
        ...
        foreach ($list as $prd) {
            ...
            $row[] = $no;
            $pid = $prd->pid;
            $row[] = $prd->product_name;//'<a href="#" data-object-id="' . $pid . '" class="view-object"><span class="avatar-lg align-baseline"><img src="' . base_url() . 'userfiles/product/thumbnail/' . $prd->image . '"  width="40" height="40"></span></a>';
            $row[] = +$prd->qty;
            ...
            $row[] = 'IDR '.number_format($prd->fproduct_price, 2, ".", ".");
            $row[] = '<a href="#" data-object-id="' . $pid . '" class="btn btn-success  btn-sm  view-object"><span class="fa fa-eye"></span> ' . $this->lang->line('View') . '</a> 
                                    <div class="btn-group">
                                    <button type="button" class="btn btn-warning text-white dropdown-toggle btn-sm" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-print"></i>  ' . $this->lang->line('Print') . '</button>
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="' . base_url() . 'products/barcode?id=' . $pid . '" target="_blank"> ' . $this->lang->line('BarCode') . '</a><div class="dropdown-divider"></div> '
                    . '<a class="dropdown-item" href="' . base_url() . 'products/posbarcode?id=' . $pid . '" target="_blank"> ' . $this->lang->line('BarCode') . ' - Compact</a> <div class="dropdown-divider"></div>
                    ...
                    . '                 <a type="button" class="btn btn-danger  btn-sm" href="' . base_url() . 'products/report_product?id=' . $pid . '" target="_blank"> <span class="fa fa-pie-chart"></span> ' . $this->lang->line('Reports') . '</a> <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu">
                                    &nbsp;<a href="' . base_url() . 'products/edit?id=' . $pid . '"  class="btn btn-warning text-white btn-sm"><span class="fa fa-edit"></span>' . $this->lang->line('Edit') . '</a><div class="dropdown-divider"></div>&nbsp;<a href="#" data-object-id="' . $pid . '" class="btn btn-danger btn-sm  delete-object"><span class="fa fa-trash"></span>' . $this->lang->line('Delete') . '</a>
                                    </div>
                                </div>';
            $data[] = $row;
        }
    ...
    public function delete_i() {
        if ($this->aauth->premission(11)) {
            ...
            if ($id) {
                $this->db->delete('smartpos_products', array('pid' => $id));
                $this->db->delete('smartpos_products', array('sub' => $id, 'merge' => 1));
                $this->db->delete('smartpos_movers', array('d_type' => 1, 'rid1' => $id));
                $this->db->set('merge', 0);
    ...
    public function view_over() {
        ...
        $this->db->select('smartpos_products.*,smartpos_warehouse.title');
        $this->db->from('smartpos_products');
        $this->db->where('smartpos_products.pid', $pid);
        ...
        $this->db->where('smartpos_products.merge', 1);
        $this->db->where('smartpos_products.sub', $pid);
        $query = $this->db->get();
        ...
        $this->db->where('smartpos_products.sub', $pid);
        $this->db->where('smartpos_products.merge', 2);
        $query = $this->db->get();
    ...
    public function report_product() {
        ...
        if ($pid && $r_type) {
            ...
            switch ($r_type) {
                case 1 :
                    $query = $this->db->query("SELECT smartpos_invoices.tid,smartpos_invoice_items.qty,smartpos_invoice_items.price,smartpos_invoices.invoicedate FROM smartpos_invoice_items LEFT JOIN smartpos_invoices ON smartpos_invoices.id=smartpos_invoice_items.tid WHERE smartpos_invoice_items.pid='$pid' AND smartpos_invoices.status!='canceled' AND (DATE(smartpos_invoices.invoicedate) BETWEEN DATE('$s_date') AND DATE('$e_date'))");
                    $result = $query->result_array();
                    break;

                case 2 :
                    $query = $this->db->query("SELECT smartpos_purchase.tid,smartpos_purchase_items.qty,smartpos_purchase_items.price,smartpos_purchase.invoicedate FROM smartpos_purchase_items LEFT JOIN smartpos_purchase ON smartpos_purchase.id=smartpos_purchase_items.tid WHERE smartpos_purchase_items.pid='$pid' AND smartpos_purchase.status!='canceled' AND (DATE(smartpos_purchase.invoicedate) BETWEEN DATE('$s_date') AND DATE('$e_date'))");
                    $result = $query->result_array();
                    break;

                case 3 :
                    $query = $this->db->query("SELECT rid2 AS qty, DATE(d_time) AS  invoicedate,note FROM smartpos_movers  WHERE smartpos_movers.d_type='1' AND rid1='$pid'  AND (DATE(d_time) BETWEEN DATE('$s_date') AND DATE('$e_date'))");
                    $result = $query->result_array();
                    break;
            }
    ...
    public function custom_label() {
        if ($this->input->post()) {
            ...
            foreach ($this->input->post('products_l') as $row) {
                $this->db->select('smartpos_products.product_name,smartpos_products.product_price,smartpos_products.product_code,smartpos_products.barcode,smartpos_products.expiry,smartpos_products.code_type,smartpos_warehouse.title,smartpos_warehouse.loc');
                $this->db->from('smartpos_products');
                $this->db->join('smartpos_warehouse', 'smartpos_warehouse.id = smartpos_products.warehouse', 'left');
                ...
                //  $this->db->where('warehouse', $warehouse);
                $this->db->where('smartpos_products.pid', $row);
                $query = $this->db->get();
                $resultz = $query->row_array();
