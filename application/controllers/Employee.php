...
class Employee extends CI_Controller {
    ...
    function delete_user() {
        ...
        if ($nuid == $uid) {
            ...
            $this->db->delete('smartpos_employees', array('id' => $uid));

            $this->db->delete('smartpos_users', array('id' => $uid));
    ...
    public function permissions_update() {
        ...
        foreach ($permission as $row) {
            ...
            $this->db->set($data);
            $this->db->where('id', $i);
            $this->db->update('smartpos_premissions');
        }
    ...
    public function hday_list() {
        ...
        foreach ($list as $obj) {
            ...
            $row[] = $day;
            $row[] = $obj->val3;
            $row[] = "<a href='" . base_url("employee/editholiday?id=$obj->id") . "' class='btn btn-success btn-sm'><i class='fa fa-pencil'></i> " . "</a> " . '<a href="#" data-object-id="' . $obj->id . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
    ...
    public function emp_search() {
        ...
        if ($this->aauth->get_user()->loc) {
            $whr = ' (smartpos_users.loc=' . $this->aauth->get_user()->loc . ') AND ';
        }
        if ($name) {
            $query = $this->db->query("SELECT smartpos_employees.* ,smartpos_users.email FROM smartpos_employees  LEFT JOIN smartpos_users ON smartpos_users.id=smartpos_employees.id  WHERE $whr (UPPER(smartpos_employees.name)  LIKE '%" . strtoupper($name) . "%' OR UPPER(smartpos_employees.phone)  LIKE '" . strtoupper($name) . "%') LIMIT 6");
    ...
    public function payrolllist() {
        ...
        foreach ($list as $prd) {
            ...
            $row[] = $prd->payer;
            $row[] = $prd->method;
            $row[] = '<a href="' . base_url() . 'transactions/view?id=' . $pid . '" class="btn btn-primary btn-sm"><span class="fa fa-eye"></span></a> <a  href="#" data-object-id="' . $pid . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a> ';
            $data[] = $row;
        }
