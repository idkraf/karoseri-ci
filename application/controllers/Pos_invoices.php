...
class Pos_invoices extends CI_Controller {
    ...
    public function action() {
        ...
        if ($ptype == 4) {
            ...
            if ($this->db->insert('smartpos_invoices', $data)) {
                ...
                if ($prodindex > 0) {
                    ...
                    if (@count($product_serial) > 0) {
                        ...
                        $this->db->update('smartpos_product_serials');
                    }
                } else {
                    echo json_encode(array('status' => 'Error', 'message' =>
                ...
                if ($transok) {
                    ...
                    echo json_encode(array('status' => 'Success', 'message' =>
                        $this->lang->line('Invoice Success') . "<br><br>"
                        . "<div class='row'>"
                        . "<div class='col-md-1'>"
                        . "<div class='card border border-1 border-success rounded shadow'>"
                        . "<div class='card-body text-center'>"
                        . "<a target='_blank' href='thermal_pdf?id=$invocieno' class='btn btn-light p-0'>"
                            . "<img src='". base_url("assets/images/pdf-40.png")."'> <p>PDF</p>  </a>"
                        . "</div>"
                        . "</div>"
                        . "</div>"
                        . "<div class='col-md-2'>"
                        . "<div class='card border border-1 border-success rounded shadow'>"
                        . "<div class='card-body text-center'>"
                        . "<a id='$p_tid' data-ptid='$invocieno' data-url='" . $printer['val3'] . "' class='btn btn-light p-0'>"
                        . "<img src='". base_url("assets/images/print-40.png")."'><p>" . $this->lang->line('Thermal Printer') . "</p></a>"
                        . "</div>"
                        . "</div>"
                        . "</div>"
                        . "<div class='col-md-2'>"
                        . "<div class='card border border-1 border-success rounded shadow'>"
                        . "<div class='card-body text-center'>"
                        . "<a href='#' class='btn btn-light p-0' id='print_image' data-inid='$invocieno'>"
                            . "<img src='".base_url("assets/images/print-40.png")."'><p>Legacy Mode</p></a>"
                        . "</div>"
                        . "</div>"
                        . "</div>"
                        . "<div class='col-md-1'>"
                        . "<div class='card border border-1 border-success rounded shadow'>"
                        . "<div class='card-body text-center'>"
                        . "<a target='_blank' href='printinvoice?id=$invocieno' class='btn btn-light p-0'>"
                            . "<img src='".base_url("assets/images/print-40.png")."'> <p>A4</p></a> "
                        . "</div>"
                        . "</div>"
                        . "</div>"
                        . "<div class='col-md-1'>"
                        . "<div class='card border border-1 border-success rounded shadow'>"
                        . "<div class='card-body text-center'>"
                        . "<a href='view?id=$invocieno' class='btn btn-light p-0'>"
                        . "<img src='".base_url("assets/images/view-40.png")."'><p>" . $this->lang->line('View') . "</p></a>"
                        . "</div>"
                        . "</div>"
                        . "</div>"
                        . "<div class='col-md-1'>"
                        . "<div class='card border border-1 border-success rounded shadow'>"
                        . "<div class='card-body text-center'>"
                        . "<a href='$link' class='btn btn-light p-0'>"
                        . "<img src='".base_url("assets/images/view-40.png")."'><p>" . $this->lang->line('Public View') . "</p></a>"
                        . "</div>"
                        . "</div>"
                        . "</div>"
                        . "<div class='col-md-1'>"
                        . "<div class='card border border-1 border-success rounded shadow'>"
                        . "<div class='card-body text-center'>"
                        . "<a href='create' class='btn btn-light p-0'>"
                        . "<img src='".base_url("assets/images/plus-40.png")."'><p> " . 'New' . "</p>  </a>"
                        . "</div>"
                        . "</div>"
                        . "</div>"
                        . "</div>"
                ...
                if ($d_trans['key2']) {
                    ...
                    $this->db->insert('smartpos_transactions', $t_data);
                    //account update
            ...
            if ($this->db->insert('smartpos_draft', $data)) {
                $invocieno2 = $invocieno;
                $invocieno = $this->db->insert_id();
                $pid = $this->input->post('pid');
                $productlist = array();
                ...
                if ($prodindex > 0) {
                    $this->db->insert_batch('smartpos_draft_items', $productlist);
                    $this->db->set(array('discount' => rev_amountExchange_s(amountFormat_general($total_discount), $currency, $this->aauth->get_user()->loc), 'tax' => rev_amountExchange_s(amountFormat_general($total_tax), $currency, $this->aauth->get_user()->loc), 'items' => $itc));
                    $this->db->where('id', $invocieno);
                    $this->db->update('smartpos_draft');
                } else {
                    echo json_encode(array('status' => 'Error', 'message' =>
                        "Please choose product from product list. Go to Item manager section if you have not added the products."));
                    $transok = false;
        ...
        if ($transok) {
            ...
            if ($draft_id > 0) {
                $this->db->delete('smartpos_draft', array('id' => $draft_id));
                $this->db->delete('smartpos_draft_items', array('tid' => $draft_id));
            }
        }
        //profit calculation
        $t_profit = 0;
        ...
        $this->db->where('smartpos_invoice_items.tid', $invocieno);
        $query = $this->db->get();
    ...
    public function extended_ajax_list() {
        ...
        foreach ($list as $invoices) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = '<a class="badge badge-success" href="' . base_url("pos_invoices/view?id=$invoices->id") . '">&nbsp; ' . $invoices->tid . '</a>';
            $row[] = $invoices->name;
            $row[] = dateformat($invoices->invoicedate);
    ...
    public function editaction() {
        ...
        if ($ptype == 4) {
            ...
            $this->db->set($data);
            $this->db->where('id', $invocieno);
            if ($this->db->update('smartpos_invoices', $data)) {
                //Product Data
                $pid = $this->input->post('pid');
                $productlist = array();
                $prodindex = 0;
                $itc = 0;
                $this->db->delete('smartpos_invoice_items', array('tid' => $invocieno));
                $product_id = $this->input->post('pid');
                $product_name1 = $this->input->post('product_name', true);
                $product_qty = $this->input->post('product_qty');
        ...
        $this->db->update('smartpos_metadata');
    }
    ...
    function thermal_print($id = 0, $mode = true, $d = true) {
        ...
        if ($printer_data) {
            ...
            if ($printer_data['val2'] != 'server') {
                ...
                $this->db->select('key');
                $this->db->from('smartpos_restkeys');
                $this->db->limit(1);
                $query_r = $this->db->get();
