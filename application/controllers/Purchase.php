...
class Purchase extends CI_Controller {
    ...
    public function index(){
        //data dari tabel smartpos_purchase - 
        //buat baru tabel purchase dan purchase_item
        //
    ...
    public function ajax_list(){
        ...
        foreach ($list as $prd) {
            ...
            foreach ($product as $srv) {
                ...
                $row2['size'] = $srv['size'];
                $row2['big'] = $srv['big'];
                $row2['indent'] = $srv['indent'];
                $row2['pricebig'] = $srv['pricebig'];
                $row2['disc'] = $srv['disc'];
                $row2['tax'] = $srv['tax'];
                $row2['subtotal'] = $srv['subtotal'];
            ...
            $row['code2'] = $prd->code2;
            $row['code'] = $prd->code;//'<a href="' . base_url("stockreturn/view?id=$prd->id") . '" class="text-black rounded-0" style="text-decoration: none"><strong>&nbsp; ' . $prd->tid . '</strong></a>';
            $row['date'] = dateformat($prd->date);
            $row['datedue'] = dateformat($prd->datedue);
            $row['supplier_name'] = $prd->name;
            $row['total'] = $total;
            $row['payment'] = $prd->payment;
    ...
    public function purchase_insert(){
        //simpen ke table purchase dan purchase_item
        //table:purchase
        //

        //table:purchase_item
        //id, product_id, indent, qty, big, pricebig, disc, tax , subtotal
        //jika purchase_item.qty > product.qty maka indent > 0
    ...
    public function create() {
        ...
        $data['currency'] = $this->purchases->currencies();
        $this->load->model('customers_model', 'customers');
        $data['customergrouplist'] = $this->customers->group_list();
        $data['lastinvoice'] = $this->purchases->lastpurchase();
        $data['terms'] = $this->purchases->billingterms();
        $head['title'] = "New Purchase";
        $head['usernm'] = $this->aauth->get_user()->username;
        $data['warehouse'] = $this->purchases->warehouses();
    ...
    public function edit() {
        ...
        $data['customergrouplist'] = $this->customers->group_list();
        $data['terms'] = $this->purchases->billingterms();
        $data['invoice'] = $this->purchases->purchase_details($tid);
        $data['products'] = $this->purchases->purchase_products($tid);
        ;
    ...
    public function action() {
        ...
        if ($this->db->insert('smartpos_purchase', $data)) {
            ...
            if ($prodindex > 0) {
                ...
                $this->db->update('smartpos_purchase');
            } else {
                echo json_encode(array('status' => 'Error', 'message' =>
    ...
    public function view() {
        ...
        $data['invoice'] = $this->purchases->purchase_details($tid);
        $data['products'] = $this->purchases->purchase_products($tid);
        $data['activity'] = $this->purchases->purchase_transactions($tid);
        $data['attach'] = $this->purchases->attach($tid);
        $data['employee'] = $this->purchases->employee($data['invoice']['eid']);
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        if ($data['invoice']['tid'])
    ...
    public function delete_i() {
        ...
        if ($this->purchases->purchase_delete($id)) {
    ...
    public function editaction() {
        ...
        $this->db->delete('smartpos_purchase_items', array('tid' => $invocieno));
        $product_id = $this->input->post('pid');
        $product_name1 = $this->input->post('product_name', true);
        $product_qty = $this->input->post('product_qty');
        ...
        if ($flag) {
            ...
            if ($this->db->update('smartpos_purchase', $data)) {
                $this->db->insert_batch('smartpos_purchase_items', $productlist);
                echo json_encode(array('status' => 'Success', 'message' =>
    ...
    public function file_handling() {
        if ($this->input->get('op')) {
            ...
            if ($this->purchases->meta_delete($invoice, 4, $name)) {
            ...
            if ($files != '') {
                ...
                $this->purchases->meta_insert($id, 4, $files);
            }
        }
    }
