...
class Stockreturn extends CI_Controller {
    ...
    public function action() {
        ...
        if ($this->db->insert('smartpos_stock_r', $data)) {
            ...
            if ($prodindex > 0) {
                $this->db->insert_batch('smartpos_stock_r_items', $productlist);
                ...
                $this->db->update('smartpos_stock_r');
            } else {
                echo json_encode(array('status' => 'Error', 'message' =>
    ...
    public function ajax_list() {
        ...
        foreach ($list as $invoices) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = '<a href="' . base_url("stockreturn/view?id=$invoices->id") . '" class="badge badge-danger text-white rounded-0" style="text-decoration: none"><strong>&nbsp; ' . $invoices->tid . '</strong></a>';
            $row[] = $invoices->name;
            $row[] = dateformat($invoices->invoicedate);
            $row[] = amountExchange($invoices->total, 0, $this->aauth->get_user()->loc);
            $row[] = '<span class="badge st-' . $invoices->status . '">' . $this->lang->line(ucwords($invoices->status)) . '</span>';
            $row[] = '<a href="' . base_url("stockreturn/view?id=$invoices->id") . '" class="btn btn-success btn-sm"><span class="fa fa-eye"></span></a> <a href="' . base_url("stockreturn/printinvoice?id=$invoices->id") . '&d=1" class="btn btn-info btn-sm"  title="Download"><span class="fa fa-download"></span></a>&nbsp;<a href="#" data-object-id="' . $invoices->id . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
            $data[] = $row;
        }
        $output = array(
            "draw" => $_POST['draw'],
    ...
    public function editaction() {
        ...
        $productlist = array();
        $prodindex = 0;
        $this->db->delete('smartpos_stock_r_items', array('tid' => $invocieno));
        $product_id = $this->input->post('pid');
        $product_name1 = $this->input->post('product_name', true);
        $product_qty = $this->input->post('product_qty');
        ...
        $this->db->where('id', $invocieno);
        if ($flag) {
            if ($this->db->update('smartpos_stock_r', $data)) {
                $this->db->insert_batch('smartpos_stock_r_items', $productlist);
                echo json_encode(array('status' => 'Success', 'message' =>
    ...
    public function cancelorder() {
        ...
        if (($stock['i_class'] != 2 && $this->aauth->premission(2)) or ($stock['i_class'] == 2 && $this->aauth->premission(1))) {
            ...
            $this->db->delete('smartpos_transactions', array('tid' => $tid, 'ext' => 6));
            echo json_encode(array('status' => 'Success', 'message' =>
