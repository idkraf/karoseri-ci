...
class Reports extends CI_Controller {
    ...
    public function customincome() {
        ...
        if ($this->input->post('check')) {
            ...
            if ($diff < 365) {
                ...
                echo json_encode(array('status' => 'Success', 'message' => 'Calculated', 'param1' => '<button class="btn btn-warning rounded-0">Income between the dates is ' . amountExchange($income['credit'], 0, $this->aauth->get_user()->loc) . '</button>'));
            } else {
    ...
    public function customexpense() {
        ...
        if ($this->input->post('check')) {
            ...
            if ($diff < 365) {
                ...
                echo json_encode(array('status' => 'Success', 'message' => 'Calculated', 'param1' => '<b>Expense between the dates is ' . amountExchange($income['debit'], 0, $this->aauth->get_user()->loc) . '</b>'));
            } else {
                echo json_encode(array('status' => 'Error', 'message' => 'Date range should be within 365 days', 'param1' => ''));
            }
        }
    ...
    public function taxviewstatements_load() {
        ...
        if ($trans_type == 'Sales') {
            $where = " WHERE (DATE(smartpos_invoices.invoicedate) BETWEEN '$sdate' AND '$edate' )";
            if ($lid > 0)
                $where .= " AND (smartpos_invoices.loc=$lid)";
            $query = $this->db->query("SELECT smartpos_customers.taxid AS VAT_Number,smartpos_invoices.tid AS invoice_number,smartpos_invoices.total AS amount,smartpos_invoices.tax AS tax,smartpos_customers.name AS customer_name,smartpos_customers.company AS Company_Name,smartpos_invoices.invoicedate AS date FROM smartpos_invoices LEFT JOIN smartpos_customers ON smartpos_invoices.csd=smartpos_customers.id" . $where);
        } else {

            $where = " WHERE (DATE(smartpos_purchase.invoicedate) BETWEEN '$sdate' AND '$edate') ";
            if ($lid > 0)
                $where .= " AND (smartpos_invoices.loc=$lid)";
            $query = $this->db->query("SELECT smartpos_supplier.taxid AS VAT_Number,smartpos_purchase.tid AS invoice_number,smartpos_purchase.total AS amount,smartpos_purchase.tax AS tax,smartpos_supplier.name AS customer_name,smartpos_supplier.company AS Company_Name,smartpos_purchase.invoicedate AS date FROM smartpos_purchase LEFT JOIN smartpos_supplier ON smartpos_purchase.csd=smartpos_supplier.id" . $where);
        }
    ...
    public function customprofit() {
        ...
        if ($this->input->post('check')) {
            ...
            if ($diff < 365) {
                ...
                echo json_encode(array('status' => 'Success', 'message' => 'Calculated', 'param1' => 'Profit between the dates is ' . amountExchange($income['col1'], 0, $this->aauth->get_user()->loc) . ' '));
            } else {
                echo json_encode(array('status' => 'Error', 'message' => 'Date range should be within 365 days', 'param1' => ''));
            }
        }
    ...
    public function customsales() {
        ...
        if ($this->input->post('check')) {
            ...
            if ($diff < 365) {
                ...
                echo json_encode(array('status' => 'Success', 'message' => 'Calculated', 'param1' => 'Sales between the dates is ' . amountExchange($income['total'], 0, $this->aauth->get_user()->loc) . ''));
            } else {
                echo json_encode(array('status' => 'Error', 'message' => 'Date range should be within 365 days', 'param1' => ''));
            }
        }
