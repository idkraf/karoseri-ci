...
class Export extends CI_Controller
{
    ...
    private function customers()
    {
        ...
        $query = $this->db->query("SELECT name,address,city,region,country,postbox,email,phone,company FROM smartpos_customers $whr");
    ...
    private function suppliers()
    {
        ...
        $query = $this->db->query("SELECT name,address,city,region,country,postbox,email,phone,company FROM smartpos_supplier $whr");
        $this->load->dbutil();
        $this->load->helper('file');
        $this->load->helper('download');
        header('Content-Description: File Transfer');
    ...
    function transactions_o()
    {
        ...
        header('Content-Transfer-Encoding: binary');
        $query = $this->db->query("SELECT account,type,cat AS category,debit,credit,payer,method,date,note FROM smartpos_transactions" . $where . ' ' . $whr);
        echo "\xEF\xBB\xBF"; // Byte Order Mark
        echo $this->dbutil->csv_from_result($query);
    }
    ...
    function products_o()
    {
        $whr = '';
        if ($this->aauth->get_user()->loc) {
            $whr = "LEFT JOIN smartpos_warehouse ON smartpos_products.warehouse=smartpos_warehouse.id WHERE smartpos_warehouse.loc='" . $this->aauth->get_user()->loc . "';";
        } elseif (!BDATA) {
            $whr = "LEFT JOIN smartpos_warehouse ON smartpos_products.warehouse=smartpos_warehouse.id WHERE smartpos_warehouse.loc='0';";
        }
        ...
        switch ($type) {
            case 1 :
                $query = "SELECT product_name,product_code,product_price,fproduct_price AS factory_price,taxrate,disrate AS discount_rate,qty FROM smartpos_products $whr";
                break;

            case 2 :
                $query = "SELECT smartpos_product_cat.title as category,smartpos_products.product_name,smartpos_products.product_code,smartpos_products.product_price,smartpos_products.fproduct_price AS factory_price,smartpos_products.taxrate,smartpos_products.disrate AS discount_rate,smartpos_products.qty FROM smartpos_products LEFT JOIN smartpos_product_cat ON smartpos_products.pcat=smartpos_product_cat.id $whr";
                break;
        }
    ...
    function taxstatement_o()
    {
        ...
        $whr2 = '';
        if ($this->aauth->get_user()->loc) {
            $whr = " AND smartpos_invoices.loc='" . $this->aauth->get_user()->loc . "';";
            $whr2 = " AND smartpos_purchase.loc='" . $this->aauth->get_user()->loc . "';";
        } elseif (!BDATA) {
            $whr = " AND smartpos_invoices.loc='0';";
            $whr2 = " AND smartpos_purchase.loc='0';";
        }
        ...
        if ($trans_type == 'Sales') {
            $where = " WHERE (DATE(smartpos_invoices.invoicedate) BETWEEN '$sdate' AND '$edate') $whr";
            $query = $this->db->query("SELECT smartpos_customers.taxid AS TAX_Number,concat('$prefix',smartpos_invoices.tid) AS invoice_number,concat('$curr',smartpos_invoices.total) AS amount,smartpos_invoices.shipping AS shipping,smartpos_invoices.ship_tax AS ship_tax,smartpos_invoices.ship_tax_type AS ship_tax_type,smartpos_invoices.discount AS discount,smartpos_invoices.tax AS tax,smartpos_invoices.pmethod AS payment_method,smartpos_invoices.status AS status,smartpos_invoices.refer AS referance,smartpos_customers.name AS customer_name,smartpos_customers.company AS Company_Name,smartpos_invoices.invoicedate AS date FROM smartpos_invoices LEFT JOIN smartpos_customers ON smartpos_invoices.csd=smartpos_customers.id" . $where);

            echo $this->dbutil->csv_from_result($query);

        } else {

            $where = " WHERE (DATE(smartpos_purchase.invoicedate) BETWEEN '$sdate' AND '$edate') $whr";
            $query = $this->db->query("SELECT concat('$prefix',smartpos_purchase.tid) AS receipt_number,concat('$curr',smartpos_purchase.total) AS amount,smartpos_purchase.tax AS tax,smartpos_supplier.name AS supplier_name,smartpos_supplier.company AS Company_Name,smartpos_purchase.invoicedate AS date FROM smartpos_purchase LEFT JOIN smartpos_supplier ON smartpos_purchase.csd=smartpos_supplier.id" . $where);
