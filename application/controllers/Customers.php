...
class Customers extends CI_Controller {
    ...
    public function load_list() {
        ...
        if ($this->input->post('due')) {
            foreach ($list as $customers) {
                ...
                $row[] = '<span class="avatar-sm align-baseline"><img width="60" height="60" class="rounded-circle" src="' . base_url() . 'userfiles/customers/thumbnail/' . $customers->picture . '" ></span> &nbsp;<a class="badge badge-success" href="customers/view?id=' . $customers->id . '">' . $customers->name . '</a>';
                ...
                $row[] = $customers->email;
                $row[] = $customers->phone;
                $row[] = '<a href="customers/view?id=' . $customers->id . '" class="btn btn-info btn-sm"><span class="fa fa-eye"></span>  ' . '</a> <a href="customers/edit?id=' . $customers->id . '" class="btn btn-primary btn-sm"><span class="fa fa-pencil"></span>  ' . '</a> <a href="#" data-object-id="' . $customers->id . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
                $data[] = $row;
            }
        } else {
            foreach ($list as $customers) {
                ...
                $row = array();
                $row[] = $no . ' <input type="checkbox" name="cust[]" class="checkbox" value="' . $customers->id . '"> ';
                $row[] = '<span class="avatar-sm align-baseline"><img width="40" height="40" class="rounded-circle" src="' . base_url() . 'userfiles/customers/thumbnail/' . $customers->picture . '" ></span> &nbsp;<a class="badge badge-success" href="customers/view?id=' . $customers->id . '">' . $customers->name . '</a>';
                $row[] = $customers->address . ',' . $customers->city . ',' . $customers->country;
                $row[] = $customers->email;
                $row[] = $customers->phone;
                $row[] = '<a href="customers/view?id=' . $customers->id . '" class="btn btn-info btn-sm"><span class="fa fa-eye"></span>  ' . '</a> <a href="customers/edit?id=' . $customers->id . '" class="btn btn-primary btn-sm"><span class="fa fa-pencil"></span>  ' . '</a> <a href="#" data-object-id="' . $customers->id . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
    ...
    public function ajax_modal_list(){
        $cid = "";
        $list = $this->customers->get_datatable($cid);
        $data = array();
        foreach ($list as $project) {
            $row = array();
            $row[] = '<a href="#" 
            data-object-cid="' . $project->id . '" 
            data-object-ccode="' . $project->phone . '" 
            data-object-cname="' . $project->name . '" 
            data-toggle="modal" data-target="#dataCustomer" data-dismiss="modal"  
            class="btn btn-success btn-sm pilih-customer"><span class="icon-share-alt"></span></a>';
            $row[] = $project->name;
            $row[] = $project->address;
            $row[] = $project->city;
            $row[] = $project->phone;
        ...
        $output = array(
            "recordsTotal" => $this->customers->countall($cid),
            "recordsFiltered" => $this->customers->countfiltered($cid),
            "data" => $data,
        );
        //output to json format
        echo json_encode($output);
