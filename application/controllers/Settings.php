...
class Settings extends CI_Controller {
    ...
    public function delete_custom_field() {
        ...
        if ($this->db->delete('smartpos_custom_fields', array('id' => $id))) {
            $this->db->delete('smartpos_custom_data', array('field_id' => $id));
            echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('DELETED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' => $this->lang->line('ERROR')));
        }
...
FROM smartpos_invoices AS i LEFT JOIN smartpos_customers AS c ON i.csd=c.id GROUP BY  i.csd ORDER BY  i.id  LIMIT 10");
        $error = $this->db->error();
        if (@$error['code']) {
    ...
    public function businessSetting() {
        ...
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('settings/business_setting', $data);
        $this->load->view('fixed/footer');
    }
