...
{
    $ci =& get_instance();
    $query = $ci->db->query("SELECT currency FROM smartpos_system WHERE id=1 LIMIT 1");
    $row = $query->row_array();
...
function amountExchange($number, $id = 0, $loc = 0)
{
    ...
    $ci->load->database();
    if ($loc > 0 && $id == 0) {
        $query = $ci->db->query("SELECT cur FROM smartpos_locations WHERE id='$loc' LIMIT 1");
        $row = $query->row_array();
        $id = $row['cur'];
    }
    if ($id > 0) {
        $query = $ci->db->query("SELECT * FROM smartpos_currencies WHERE id='$id' LIMIT 1");
        $row = $query->row_array();
        $currency = $row['symbol'];
        $rate = $row['rate'];
        $thosand = $row['thous'];
...
function rev_amountExchange_s($number, $id = 0, $loc = 0)
{
    ...
    if ($loc) {
        ...
        $lcid = $row['cur'];
        if ($lcid > 0) {
            $query = $ci->db->query("SELECT * FROM smartpos_currencies WHERE id='$lcid' LIMIT 1");
            $row = $query->row_array();
			if($row['id']){
            $rate = $row['rate'];
...
function rev_amountExchange($number, $id = 0)
{
    ...
    $reverse = $row['other'];
    if ($reverse && $id > 0) {
        $query = $ci->db->query("SELECT rate FROM smartpos_currencies WHERE id='$id' LIMIT 1");
        $row = $query->row_array();
        $rate = $row['rate'];
...
function locations()
{
    $ci =& get_instance();
    $ci->load->database();
    $query2 = $ci->db->query("SELECT * FROM smartpos_locations");
...
function location($number = 0)
{
    ...
    $ci->load->database();
    if ($number > 0) {
        $query2 = $ci->db->query("SELECT * FROM smartpos_locations WHERE id=$number");
        return $query2->row_array();
    } else {
        $query2 = $ci->db->query("SELECT cname,address,city,region,country,postbox,phone,email,taxid,logo,foundation FROM smartpos_system WHERE id=1 LIMIT 1");
        return $query2->row_array();
    }
}
