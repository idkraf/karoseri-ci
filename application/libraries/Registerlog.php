...
class Registerlog
{
    ...
    {
        $this->RI->db->from('smartpos_register');
    ...
    {
        $this->RI->db->from('smartpos_register');
        $this->RI->db->where('smartpos_register.id', $id);
              $this->RI->db->join('smartpos_users', 'smartpos_register.uid=smartpos_users.id', 'left');
            if ($this->RI->aauth->get_user()->loc) {
            $this->RI->db->group_start();
            $this->RI->db->where('smartpos_users.loc', $this->RI->aauth->get_user()->loc);
            if (BDATA) $this->RI->db->or_where('smartpos_users.loc', 0);
            $this->RI->db->group_end();
        } elseif (!BDATA) {
            $this->RI->db->where('smartpos_users.loc', 0);
        }
        $query = $this->RI->db->get();
        $result = $query->row_array();
        if ($result) {
    ...
    public function create($id, $cash, $card, $bank, $cheque)
    {
        ...
        return $this->RI->db->insert('smartpos_register', $data);
    }
    ...
    public function update($id, $cash = 0,  $card = 0, $bank = 0, $cheque = 0,$change = 0)
    {
        ...
        $this->RI->db->where('uid', $id);
        $this->RI->db->where('active', 1);
        $this->RI->db->update('smartpos_register');
    }
    ...
    {
       $this->RI->db->select('smartpos_register.*,smartpos_users.username,smartpos_users.loc');
        $this->RI->db->from('smartpos_register');
       $this->RI->db->join('smartpos_users','smartpos_register.uid=smartpos_users.id','left');
       if($loc)    $this->RI->db->where('smartpos_users.loc',$loc);
        $query = $this->RI->db->get();
