...
class Common
{
    ...
    function taxlist($id = 0)
    {
        ...
        if ($id > 0) {
            ...
            $query = $this->PI->db->get('smartpos_config');
    ...
    function languages($id = 0)
    {
        if ($id) {
            ...
            $this->PI->db->from('smartpos_locations');
            $this->PI->db->where('id', $id);
            $query = $this->PI->db->get();
            $out = $query->row_array();
        } else {
            $this->PI->db->select('*');
            $this->PI->db->from('smartpos_system');
    ...
    public function default_warehouse()
    {
        if ($this->PI->aauth->get_user()->loc) {
            ...
            $this->PI->db->select('smartpos_locations.ware,smartpos_warehouse.title');
            $this->PI->db->from('smartpos_locations');
            $this->PI->db->join('smartpos_warehouse', 'smartpos_locations.ware=smartpos_warehouse.id', 'left');
            $this->PI->db->where('smartpos_locations.id', $this->PI->aauth->get_user()->loc);
            $query = $this->PI->db->get();
            ...
            $wr = '<option value="0">*' . $this->PI->lang->line('All') . '</option>';
            $this->PI->db->select('univarsal_api.key1,smartpos_warehouse.title');
            $this->PI->db->from('univarsal_api');
            $this->PI->db->join('smartpos_warehouse', 'univarsal_api.key1=smartpos_warehouse.id', 'left');
