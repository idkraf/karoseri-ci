<?php
/**
 * Smart POS -  Accounting,  Invoicing  and CRM Application
...
class Aauth
{
        ...
        public function premission($module_id)
    {
        ...
        $query = $this->aauth_db->get('smartpos_premissions');
        $out=$query->row_array();
        return $out[$role];
    }
    ...
    {
        $this->aauth_db->select('smartpos_pms.*,smartpos_pms.id AS pid,smartpos_employees.*');
        $this->aauth_db->from('smartpos_pms');
        if (is_numeric($receiver_id)) {
            $query = $this->aauth_db->where('smartpos_pms.receiver_id', $receiver_id);
            $query = $this->aauth_db->where('smartpos_pms.pm_deleted_receiver', 0);
        }
        if (is_numeric($sender_id)) {
            $query = $this->aauth_db->where('smartpos_pms.sender_id', $sender_id);
            $query = $this->aauth_db->where('smartpos_pms.pm_deleted_sender', 0);
        }

        $this->aauth_db->order_by('smartpos_pms.id', 'DESC');
        $this->aauth_db->join('smartpos_employees', 'smartpos_employees.id = smartpos_pms.sender_id', 'left');
        $this->aauth_db->limit($limit, $offset);
    ...
    {
         $this->aauth_db->insert('smartpos_log', array('note'=>$input1,'user'=>$input2,'created'=>date('Y-m-d H:i:s')));
    }
     ...
     public function clock()
     {
         ...
         $this->aauth_db->from('smartpos_employees');
         $query = $this->aauth_db->get();
         $emp = $query->row_array();
         return $emp['clock'];
     }
