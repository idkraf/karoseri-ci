        ...
        var flag = true;
         ...
         $('#v2_search_bar').val('');
            ...
            if (flag) {
                ...
                var data = ' <tr  id="ppid-' + cvalue + '"> \n\
<td> \n\
<div class="quantity input-group">\n\
<span class="input-group-text quantity-up btn btn-primary">+</span>\n\
...
<span class="input-group-text btn btn-danger  quantity-down">-</span>\n\
\n\
\n\
</div>\n\
</span>' + $(this).attr('data-name') + '-' + $(this).attr('data-pcode') + '</td> \n\
<td> ' + $(this).attr('data-price') + ' </td> \n\
<td>\n\
<strong><span class="ttlText" id="result-' + cvalue + '">0</span></strong>\n\
<td><a class="remove  btn btn-sm btn-danger red removeItem" data-rowid="' + cvalue + '" title="Remove"> <i class="fa fa-trash"></i> </a></td>\n\
</td>\n\
<input type="hidden" class="form-control text-center" name="product_name[]" id="productname-' + cvalue + '" value="' + $(this).attr('data-name') + '-' + $(this).attr('data-pcode') + '">\n\
<input type="hidden" id="alert-' + cvalue + '" value="' + $(this).attr('data-stock') + '"  name="alert[]"><input type="hidden" class="form-control req prc" name="product_price[]" id="price-' + cvalue + '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' + functionNum + '), billUpyog()" autocomplete="off"  value="' + $(this).attr('data-price') + '" inputmode="numeric"> \n\
...
function addObject(action, action_url) {
    ...
    var $btn;
    if ($("#notify").length == 0) {
        $("#c_body").html('<div id="notify" class="alert" style="display:none;"><a href="#" class="btn btn-danger btn-sm float-end" data-dismiss="alert">&nbsp;X&nbsp;</a><div class="message"></div></div>');
    }
    if (errorNum > 0) {
        $("#notify").removeClass("alert-success").addClass("alert-warning bg-warning").fadeIn();
        $("#notify .message").html("<strong>Error</strong>: It appears you have forgotten to complete something!");
        $("html, body").scrollTop($("body").offset().top);
    } else {
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                if (data.status == "Success") {
                    $("#notify .message").html("<strong>" + data.status + "</strong>: " + data.message);
                    $("#notify").removeClass("alert-danger").addClass("alert-success rounded-0 border border-1 border-success").fadeIn();
                    $("html, body").scrollTop($("body").offset().top);
                    $("#data_form").remove();
                } else {
                    $("#notify .message").html("<strong>" + data.status + "</strong>: " + data.message);
